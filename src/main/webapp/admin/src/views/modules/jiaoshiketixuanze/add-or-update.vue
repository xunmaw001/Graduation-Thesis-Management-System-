      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="课题编号" prop="ketibianhao">
          <el-input v-model="ruleForm.ketibianhao" 
              placeholder="课题编号" clearable  :readonly="ro.ketibianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="课题编号" prop="ketibianhao">
              <el-input v-model="ruleForm.ketibianhao" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jiaoshiketixuanze/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiaoshiketixuanzeCrossAddOrUpdateFlag = false;
                  this.parent.search();
