      ...
      <el-row>
          ...
          <el-form-item class="input" label="学生姓名" prop="xueshengxingming">
              <el-input v-model="ruleForm.xueshengxingming" 
                placeholder="学生姓名" readonly></el-input>
                                                                  ...
                                                                  <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="申请附件" prop="shenqingfujian">
          <file-upload
          tip="点击上传申请附件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shenqingfujian" label="申请附件" prop="shenqingfujian">
            <el-button type="text" size="small" @click="download(ruleForm.shenqingfujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                        ...
                                                                        // 多级联动参数
                                                                                            info(id) {
      this.$http({
        url: `mianlunwenshenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.shenqingneirong = this.ruleForm.shenqingneirong.replace(reg,'../../../ssmb615y/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `mianlunwenshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.mianlunwenshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
